...
## Dependencies
When you need a new package, add it to `requirements.txt` or `pyproject.toml` before importing it.
...
If you realize this, create a new .md file in the `docs/developer/` folder, like `docs/developer/<pattern-name>.md`, but keep it empty for now. 
The user will prompt you to fill it with the description later with something like "Fill the description for docs/developer/<filename>.md" so the empty file serves as a context placeholder. 
...
When you encounter a common error, unexpected behavior, or debugging tip while working on this project, add it to `docs/developer/brevitas-pitfalls.md`. 
