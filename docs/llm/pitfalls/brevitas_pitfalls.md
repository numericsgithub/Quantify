...
## 17. "Maximize Unique Quantized Values" Is the Wrong Calibration Objective for Activations — It Clips Outliers

**When this happens:** Weight quantizers calibrate correctly, but activation quantizers destroy accuracy in real (non-synthetic) evaluation, because rare outlier activations get clamped almost to zero while the dense bulk of the distribution is finely resolved.

**The Problem:** `find_optimal_lsb` (`quantizers/fixedpoint_per_tensor.py`) used to select the LSB (step size) that **maximizes the number of unique quantized output values** for both weights and activations, with `prefer_high_lsb=True` for `quantizer_role == "activation"` only breaking *ties* toward a wider range. This objective is structurally biased against outliers: for a typical activation distribution (dense cluster near zero + a handful of rare extreme values), a **narrow/fine** grid resolves the dense bulk into far more distinct codes than a **wide/coarse** grid does — so it wins the "maximize unique count" comparison outright, not just on a tie. The tie-break for activations never fires in this common case, so outliers get clamped to the edge of a much-too-narrow range. Verified empirically: for a batch of `N(0, 0.1)` values plus two `±5.0` outliers quantized to 8 bits, the old objective clamped the outliers to `±0.25` (an error of ~4.75) while resolving the bulk to 256 unique codes.

**How to Prevent It:** For `prefer_high_lsb=True` (activations), `find_optimal_lsb` now uses a **coverage-first** rule instead of maximizing unique-value count: pick the *finest* (lowest) LSB whose representable range still covers the observed `abs_max` without clipping it. This guarantees the full observed range — including outliers — survives calibration, at the cost of coarser resolution on the dense bulk (an explicit, correct tradeoff for activations, where saturating a real outlier corrupts every downstream computation that depends on it, unlike weights where finer resolution of the bulk is usually more valuable). Weight calibration (`prefer_high_lsb=False`) is unchanged — it still maximizes unique-value count, ties broken by SAD.

Note this remains a **single-batch** calibration: `search_done` flips to `True` after the very first forward pass that reaches an uncalibrated quantizer (`quantizers/base_quantizer.py`), so whichever batch triggers calibration must itself contain the worst-case outlier for this fix to help — there is no multi-batch min/max aggregation across `calibration_batches` today. If outliers appear only in later calibration batches, they will still be clipped.

## 18. `disable_quantization()` Used To Compute And Discard a Full Quantized Tensor On Every Forward Call

**When this happens:** `QuantizerManager().disable_quantization()` (float warmup, or any code that wants a model to run in plain float) is active, and the model does a forward pass — with the default `quantization_start_gap=0`, on every single one.

**The Problem:** `disable_quantization()` only set `annealing_alpha=0` (and `annealing_alpha_step=0`). It did **not** stop `BaseQuantizer.forward()` from running `_calibrate()` (once) and then `_quantize()` (scale/round/clamp — real, non-trivial compute) on *every* forward call; the gate that's actually supposed to short-circuit `forward()` before any of that (`inference_counter < inference_sequence_id * quantization_start_gap`) never fires here because `quantization_start_gap` defaults to `0`, so `0 < 0` is always `False`. The fully-computed fake-quantized tensor was then thrown away by `AnnealingBlendFn.apply(x, quantized, alpha=0.0)`, which blends 100% back to the original float `x` — so the *output* was correct, but the quantize math ran and was discarded, every call. Measured on a real ~1,700-parameter, 18-quantizer model (RTX 4090, batch=256): plain PyTorch 0.841s/epoch → Brevitas module wrapping only 0.949s → permanently gated (never quantizes) 1.332s → `disable_quantization()` as it worked before this fix 2.316s. That last +0.984s was pure waste with zero effect on the output, and dwarfed any `.item()`-sync cost (pitfall context: this was found while investigating why the sync-reduction work above didn't fix a "GPU slower than CPU" report — the syncs were real but not the dominant cost for a model this small).

**How to Prevent It:** `QuantizerManager` now has a `quantization_globally_disabled` flag (`quantizers/manager.py`), set by `disable_quantization()` / cleared by `enable_quantization()`, that `BaseQuantizer.forward()` checks **first**, before the gating logic — skipping calibration and quantize compute entirely, not just discarding the result. One thing this broke on the first attempt, worth knowing if you add another method that should "really" turn quantization on: `training_harness/trainer_v2.py::_activate_qat()` (the actual QAT-activation code path) calls `QuantizerManager().set_annealing_for_n_inferences(...)` directly and **never** calls `enable_quantization()` — so `set_annealing_for_n_inferences()` also has to clear `quantization_globally_disabled` itself, or a model that started with `disable_quantization()` during float warmup would stay hard-disabled forever, silently never quantizing anything through the rest of training. See `tests/test_quantization_hard_disable.py` for the regression tests, including the specific disable → `set_annealing_for_n_inferences()` (no `enable_quantization()`) sequence.
